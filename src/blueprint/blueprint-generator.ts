...
function buildSectionPrompt(
  ...
  const sectionPrompt = AGENT_INSTRUCTIONS.SECTION_PROMPTS[sectionKey as keyof typeof AGENT_INSTRUCTIONS.SECTION_PROMPTS];
...
export class BlueprintGenerator {
  ...
  async generateSection(
    ...
    logger.info(
      `Section "${SECTION_TITLES[sectionKey]}" generated — ${wordCount} words in ${Date.now() - startTime}ms`
    );
   ...
   */
  async generateBlueprint(input: StartupInput): Promise<StartupBlueprint> {
    ...
    const sections: Record<string, BlueprintSection> = {};
  ...
  private assembleMarkdownReport(
    ...
    const sectionBodies = Object.values(sections)
      .map((section) => {
        const sourcesNote =
          section.ragSourcesUsed.length > 0
