...
import type { BlueprintGenerationProgress } from "./blueprint/blueprint-types.js";
...
function createProgressHandler(spinner: ReturnType<typeof ora>) {
  return (progress: BlueprintGenerationProgress) => {
...
async function promptForStartupInput(): Promise<StartupInput> {
  ...
  const answers = await inquirer.prompt([
    {
      ...
      validate: (val: string) =>
        val.trim().length >= 10 || "Please provide at least 10 characters.",
    },
    {
      type: "list",
      name: "industry",
  ...
  return answers as StartupInput;
}
...
program
  ...
  .action(async (opts) => {
    ...
    const input: StartupInput = {
      idea: opts.idea as string,
      industry: opts.industry as string,
      targetGeography: opts.geo as string,
      stage: opts.stage as StartupInput["stage"],
      fundingStage: opts.funding as StartupInput["fundingStage"],
    };
